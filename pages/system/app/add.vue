<template>
  <view class="uni-container">
    ...
    <uni-forms ref="form" v-model="formData" validateTrigger="bind" style="max-width: 792px" :labelWidth="labelWidth" :rules="rules">
      <uni-card title="基础信息">
        <uni-forms-item name="app_type" label="应用类型" required>
          <uni-data-select v-model="formData.app_type" :localdata="appTypeOptions" placeholder="请选择应用类型" :clear="false"> </uni-data-select>
        </uni-forms-item>
        <uni-forms-item class="forn-item__flex" name="appid" label="AppID" required>
          <uni-easyinput :disabled="isEdit" placeholder="应用的AppID" v-model="formData.appid" trim="both"> </uni-easyinput>
        </uni-forms-item>
        <uni-forms-item name="name" label="应用名称" required>
          <uni-easyinput :disabled="isEdit" placeholder="应用名称" v-model="formData.name" trim="both"> </uni-easyinput>
        </uni-forms-item>
      ...
      <uni-card title="图标素材">
        <uni-forms-item label="应用图标">
          <uni-file-picker
            v-model="middleware_img.icon_url"
            :image-styles="{ width: '200rpx' }"
            return-type="object"
        ...
        <uni-forms-item label="应用截图">
          <uni-file-picker v-model="screenshotList" file-mediatype="image" mode="grid" :image-styles="{ height: '500rpx', width: '300rpx' }" @delete="iconUrlDelete">
          </uni-file-picker>
        </uni-forms-item>
      </uni-card>
      ...
      <uni-card class="app_platform" title="App 信息">
        ...
        <view v-for="item in appPlatformKeys" :key="item">
          <checkbox-group
            @change="
              ({ detail: { value } }) => {
                setPlatformChcekbox(item, !!value.length);
              }
            "
          >
          ...
          <template v-if="getPlatformChcekbox(item)">
            ...
            <uni-forms-item class="forn-item__flex" v-if="item === 'app_android'" label="上传apk包">
              <uni-file-picker
                v-model="appPackageInfo"
                file-extname="apk"
                :disabled="hasPackage"
                :provider="uniFilePickerProvider"
                returnType="object"
                file-mediatype="all"
                limit="1"
                @success="(res) => iconUrlSuccess(res, `${item}.url`)"
                @delete="(res) => iconUrlDelete(res, `${item}.url`)"
                style="flex: 1"
              >
                <view class="flex">
                  <radio-group @change="(e) => (this.uniFilePickerProvider = e.detail.value)">
                    <view class="flex" style="flex-wrap: nowrap">
                      ...
                      <label style="margin-left: 20rpx"> <radio value="extStorage" /><text>扩展存储</text> </label>
                    </view>
                  </radio-group>
                  <button type="primary" size="mini" @click="selectFile" style="margin: 0 0 0 20rpx">选择文件</button>
                  <text style="padding: 10px; font-size: 12px; color: var(--color-text-secondary)"> 上传apk到当前服务空间的云存储中，上传成功后，会自动使用云存储地址填充下载链接 </text>
                </view>
              </uni-file-picker>
              <text v-if="hasPackage" style="padding-left: 20px; color: var(--color-text-tertiary)">
        ...
        <uni-popup ref="scheme">
          <view class="popup-content">
            <text style="font-size: 15px; font-weight: bold"> 常见的应用商店 scheme 地址 </text>
            <view></view>
            <text>
              应用宝：tmast://appdetails?r=XXX&pname=xxx； 小米：mimarket://details?id=com.xx.xx； 三星：samsungapps://ProductDetail/com.xx.xx；
              华为：appmarket://details?id=com.xx.xx； oppo：oppomarket://details?packagename=com.xx.xx； vivo：vivomarket://details?id=com.xx.xx；
            </text>
          </view>
        </uni-popup>
        <uni-forms-item name="store_schemes" label="Android应用市场" labelWidth="120">
          <view style="height: 100%">
            <view class="flex" style="justify-content: end">
              <text class="pointer" style="text-decoration: underline; color: var(--color-text-secondary); font-size: 12px; padding-left: 10rpx" @click="schemeDemo">常见应用商店schema汇总</text>
      ...
      <uni-card title="web信息">
        <uni-forms-item label="链接地址">
          ...
          <text style="font-size: 13px; color: var(--color-text-tertiary)"
            >如需免费的前端网页托管，请开通 <text style="color: inherit" class="a-link" @click="openLink('https://unicloud.dcloud.net.cn')">uniCloud</text> ，创建服务空间，并在 “前端网页托管” 里上传你的网页</text
          >
        </uni-forms-item>
      </uni-card>
