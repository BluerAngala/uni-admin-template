...
<script>
  ...
  import { fieldsMap, resFieldsMap, entFieldsMap } from './fieldsMap.js';
  ...
  export default {
    ...
    computed: {
      pageSize() {
        const { pageSizeRange, pageSizeIndex } = this.options;
    ...
    methods: {
      ...
      getPanelData() {
        const { appid, platform_id, version_id, channel_id } = this.query;
        ...
        const subTable = db
          ...
          .where(query)
          .field(`${stringifyField(fieldsMap)},dimension,stat_date`)
